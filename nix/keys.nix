...
let
  ...
  convertOldKeyType = key: val: let
    ...
    + " \"<value>\"`.";
  in
    if isString val then builtins.trace warning { text = val; } else val;
  ...
  keyType = mkOptionType {
    ...
    in
      keyOptionsType.merge loc (map convert defs);
...

{
  ...
  config = {
    ...
    warnings = mkIf config.deployment.storeKeysOnMachine [
      (
        "The use of `deployment.storeKeysOnMachine' imposes a security risk " + "because all keys will be put in the Nix store and thus are world-" + "readable. Also, this will have an impact on services like OpenSSH, " + "which require strict permissions to be set on key files, so expect " + "things to break."
      )
    ];

    assertions = flip mapAttrsToList config.deployment.keys (
      key: opts: {
        assertion = (opts.text == null && opts.keyFile != null) || (opts.text != null && opts.keyFile == null);
    ...
    system.activationScripts.nixops-keys =
      let
        ...
        '';
      in
        stringAfter [ "users" "groups" ] "source ${pkgs.writeText "setup-keys.sh" script}";
    ...
    systemd.services = (
      ...
      // (
        flip mapAttrs' config.deployment.keys (
          name: keyCfg:
