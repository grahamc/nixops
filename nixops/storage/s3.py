...
class S3Backend:
    ...
    def fetchToFile(self, path: str, **kwargs):
        ...
        try:
            ...
            if e.response['Error']['Code'] == '404':
                self.aws.client('s3').put_object(Bucket=self.bucket,
                              Key=self.key,
                              Body=b'',
                              **self.encargs()
                              )

    def onOpen(self, sf: nixops.statefile.StateFile):
        pass
    ...
    def uploadFromFile(self, path: str, **kwargs) -> bool:
        with open(path, 'rb') as f:
            self.aws.client('s3').upload_fileobj(f, self.bucket, self.key, ExtraArgs=self.encargs())

        self.unlock(path)

    def s3(self) -> None:
        self.aws.client('s3', endpoint_url=self.endpoint)

    def encargs(self) -> Dict[str, str]:
        if self.kms_keyid is not None:
            return {
                "ServerSideEncryption": 'aws:kms',
                "SSEKMSKeyId": self.kms_keyid
            }
        else:
            return {}
