...
class S3Backend:
    ...
    def __init__(self, args: StorageArgValues) -> None:
        ...
        profile: Optional[str] = args.get("profile")
        if self.region is not None:
            self.aws = boto3.Session(region_name=self.region, profile_name=profile)
        else:
    ...
    def s3(self):
        if self.s3_endpoint is None:
            return self.aws.client("s3")
        else:
            return self.aws.client("s3", endpoint_url=self.s3_endpoint)
